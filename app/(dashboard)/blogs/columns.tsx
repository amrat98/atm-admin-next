...
//   },
  ...
  },
  {
    ...
    cell: ({ row }) => <span className="inline-block w-2xs whitespace-normal">{row.original.title || "-"}</span>,
    enableHiding: false,
  },
  {
    accessorKey: "shortDescription",
    header: ({ column }) => <DataTableColumnHeader column={column} title={columnNames["shortDescription"]} />,
    cell: ({ row }) => <p className="w-lg whitespace-normal">{row.original.shortDescription || "-"}</p>,
    enableSorting: false,
    // enableHiding: false,
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      ...
      <span>{row.original.redirectionLink.substring(0, 30) + " ..." || "-"}</span>
  ...
  },
  {
    ...
    cell: ({ row, table }) => {
      return (
        ...
        <EditBlog row={row} table={table} />
        <DeleteBlog row={row} table={table} />
        </div>
      );
    },
    enableSorting: false,
