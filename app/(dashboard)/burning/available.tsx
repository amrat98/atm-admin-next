...
import { recentUsersColumns, columnNames } from "./columns.available"

type UserExcelRow = {
    _id: string | number;
    address: string;
    availableToBurn: number;
    planAmount: number;
    planId: string;
    username: string;
    expierdOn?: string | Date | null;
};
...
export default function Burning() {
    ...
    const getData = async (opts?: { pageIndex?: number; pageSize?: number; sortKey?: string; sortBy?: 'ASC' | 'DSC'; search?: string }) => {
        ...
        try{
            ...
            const response = await axios.get(apiConfig.availableBurn, {
                headers,
                params: {
    ...
    const handleDownload = async () => {
        try {
            ...
            const { count } = await exportToExcel<UserExcelRow>({
                ...
                sheetName: "available",
                filename: `available_export_${new Date().toISOString().slice(0,10)}`,
                mapRow: (r) => ({
                    "Username": r.username || " - ",
                    "Address": r.address || " - ",
                    "Plan ID": r.planId || " - ",
                    "Plan Amount": r.planAmount || " - ",
                    "Available to Burn": r.availableToBurn || " - ",
                    "Expiry Date": r.expierdOn ? new Date(r.expierdOn).toLocaleString() : "",
                }),
            });
            if (!count) {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 *:data-[slot=card]:shadow-xs">
        <Card>
            <CardHeader>
            <CardTitle className="mb-4">Available To Burn Amount</CardTitle>
