...
import { recentUsersColumns, columnNames } from "./columns.burned"
...
export default function Burning() {
    ...
    const getData = async (opts?: { pageIndex?: number; pageSize?: number; sortKey?: string; sortBy?: 'ASC' | 'DSC'; search?: string }) => {
        ...
        try{
            ...
            const response = await axios.get(apiConfig.burnedBurn, {
                headers,
                params: {
    ...
    const handleDownload = async () => {
        try {
            ...
            const { count } = await exportToExcel<UserExcelRow>({
                ...
                sheetName: "burned",
                filename: `burned_export_${new Date().toISOString().slice(0,10)}`,
                mapRow: (r) => ({
                    "Username": r.username || " - ",
                    "Address": r.address || " - ",
                    "Plan ID": r.planId || " - ",
                    "Plan Amount": r.planAmount || " - ",
                    "Burned Amount": r.availableToBurn || " - ",
  ...
  return (
    <div className="grid grid-cols-1 gap-4 *:data-[slot=card]:shadow-xs">
        <Card>
            <CardHeader>
            <CardTitle className="mb-4">Burned Amount</CardTitle>
