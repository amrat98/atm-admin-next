...
export default function PoolUsers() {
    ...
    const getData = async (opts?: { pageIndex?: number; pageSize?: number; sortKey?: string; sortBy?: 'ASC' | 'DSC'; search?: string }) => {
        ...
        try{
            ...
            const response = await axios.get(apiConfig.listPoolUser, {
                headers,
                params: {
    ...
    const handleDownload = async () => {
        try {
            ...
            const { count } = await exportToExcel<UserExcelRow>({
                ...
                sheetName: "Pool Users",
                filename: `pool_users_export_${new Date().toISOString().slice(0,10)}`,
                mapRow: (r) => ({
                    Username: r.userName,
                    "Sponsor ID": r.sponsorName,
                    "Referral ID": r.refCode,
                    "Invitation ID": r.invCode,
                    "Children": r.child,
                    "Status": r.status ? "Activated" : "Not Activated",
  ...
  return (
    <div className="flex flex-col gap-4 md:gap-6">
        <div>
            <h1 className="text-xl font-semibold">Pool Users Management</h1>
