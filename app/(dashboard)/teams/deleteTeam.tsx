...
export function DeleteTeam({row,table}: { row: Row<z.infer<typeof recentUserSchema>>; table: Table<z.infer<typeof recentUserSchema>>;}){
  const { token } = useUser();

  const handleDeleteTeam = async (teamId: string) => {
    try {
      const headers = { token: token };
      await axios.delete(`${apiConfig.teams.delete}?_id=${teamId}`, { headers });
      ...
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            className="text-sm"
            onClick={async () => {
              const ok = await handleDeleteTeam(row.original._id);
              if (ok) {
                try {
                  table.setSorting([]);
