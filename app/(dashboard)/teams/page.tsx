...
export default function Teams() {
    ...
    const getData = async (opts?: { pageIndex?: number; pageSize?: number; sortKey?: string; sortBy?: 'ASC' | 'DSC'; search?: string }) => {
        ...
        try{
            ...
            const response = await axios.get(apiConfig.teams.get, {
                headers,
                params: {
    ...
    const handleDownload = async () => {
        try {
            ...
            const { count } = await exportToExcel<UserExcelRow>({
                ...
                sheetName: "Teams",
                filename: `teams_export_${new Date().toISOString().slice(0,10)}`,
                mapRow: (r) => ({
                    "Name": r.title,
                    "Designation": r.shortDescription,
                    "Thumbnail": r.thumbnail,
                    "Link": r.redirectionLink,
                    "Status": r.status,
