...
export default function Withdraw() {
    ...
    const [transacionStatus, setTransacionStatus] = useState("WAITING_APPROVAL");

    // Store previous filter values to detect changes
    const prevFiltersRef = useRef({ globalFilter: "", transacionType: "", transacionStatus: "" });

    const getData = async (opts?: { pageIndex?: number; pageSize?: number; sortKey?: string; sortBy?: 'ASC' | 'DSC'; search?: string; transacionType?: string; transacionStatus?: string }) => {
        if (!token) return;
        setLoading(true);
        try{
            const headers = {
            ...
            const response = await axios.get(apiConfig.getWithdrawList, {
                headers,
                params: {
                    ...
                    status: opts?.transacionStatus,
                },
            });
            //const successMessage = response.data?.responseMessage || "Data Fetch Successful";
            const meta = response.data?.result?.[0]?.metadata?.[0] || response.data?.result?.metadata?.[0];
            if (meta) {
                setTotalPages(meta.total_page);
                setTotalRows(meta.total);
            }
            setData(response.data?.result?.[0]?.data || response.data?.result?.data || [])
    ...
    const handleDownload = async () => {
        try {
            ...
            const { count } = await exportToExcel<UserExcelRow>({
                ...
                sheetName: "Withdraw",
                filename: `withdraw_export_${new Date().toISOString().slice(0,10)}`,
                mapRow: (r) => ({
                    ...
                    "Transaction Fee": r.transactionFee || " - ",
                    "Status": r.transacionStatus || " - ",
                    "Remark": r.remark || " - ",
    ...
    }, [globalFilter, paginationState.pageIndex, token, table, transacionType,transacionStatus]);
    ...
    useEffect(() => {
        ...
        getData({
            ...
            transacionStatus: transacionStatus || undefined,
        });
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [paginationState.pageIndex, paginationState.pageSize, sortingState, globalFilter, token, transacionType, transacionStatus]);
  ...
  return (
    <div className="flex flex-col gap-4 md:gap-6">
        <div>
            <h1 className="text-xl font-semibold">Withdraw Management</h1>
            <p className="text-muted-foreground text-sm">Monitor all platform withdraw and transfers</p>
        </div>
        <div className="grid grid-cols-1 gap-4 *:data-[slot=card]:shadow-xs">
            <Card>
                ...
                <Select value={transacionStatus ?? ""} onValueChange={e => {setTransacionStatus(e); table.setPageIndex(0);}}>
                    <SelectTrigger className="w-full h-10! md:max-w-50 text-sm">
                        <SelectValue placeholder="Select Status" />
                    </SelectTrigger>
                    <SelectContent>
                    {["COMPLETED", "WAITING_APPROVAL","CANCELLED"].map((p) => (
                        <SelectItem key={p} value={p}>
                ...
                <DataTablePagination table={table} totalPagesCount={totalPages} showSelectedRow={true} />
